...
import { REDIRECT_TO_LOCATION } from '../config/localStorage';
...
export default function Login() {
  ...
  useEffect(() => {
    async function loginUser(code: string) {
      try {
        await login(code);
        const redirectTo = localStorage.getItem(REDIRECT_TO_LOCATION);

        if (redirectTo) {
          localStorage.removeItem(REDIRECT_TO_LOCATION);
